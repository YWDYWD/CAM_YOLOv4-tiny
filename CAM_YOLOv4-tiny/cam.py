...
class YOLO(object):
    _defaults = {
        "model_path"        : '/home/xuqan/视频/YWD/yolov4-tiny-pytorch-master/belt论文数据汇总/0/ep080-loss3.300-val_loss4.139.pth',
    ...
    def generate(self):
       ...
       # self.net = YoloBody(len(self.anchors[0]), len(self.class_names))
        self.net    = YoloBody(self.anchors_mask, self.num_classes,5)
        #---------------------------------------------------#
        #   载入yolov4模型的权重
        #---------------------------------------------------#
        ...
        self.yolo_decodes = []
        for i in range(2):
            self.yolo_decodes.append(DecodeBox(self.anchors[i], 2,  (416, 416)))
    ...
    def detect_image(self, image):
        ...
        image_data  = np.expand_dims(np.transpose(preprocess_input(np.array(crop_img, dtype='float32')), (2, 0, 1)), 0)
...
def show_CAM(image_path, feature_maps, class_id, all_ids=7, show_one_layer=True):
            """
            ...
            layers = torch.cat([layers0, layers1], 0)
            ...
            for j in range(2):  # layers
                ...
                for idx, one in enumerate(layer_one_list):
                    ...
                    if not show_one_layer:
                        ...
                        cv2.imwrite('./'+str(j)+'layer'+str(idx)+SHOW_NAME[idx]+".jpg", show)
            ...
            if not show_one_layer:
                for idx, one_type in enumerate(all_ret):
                    ...
                    cv2.imwrite('./'+str(idx)+SHOW_NAME[idx]+".jpg", show)
...
stride = [13,26,52]
yolo = YOLO()
path = '/home/xuqan/视频/YWD/predict+cam/cam/1ab6ac62_669e_46ae_af29_b599c91e389b.jpg'
image = Image.open(path)
output_list = yolo.detect_image(image)
for i,f in enumerate(output_list):
    ret.append(f.reshape(1,3,stride[i],stride[i],7))
